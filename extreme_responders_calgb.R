...
cna_genelevel.txt <- fread((here('data','cna_genelevel.txt'))) # seg
...
######################################################## STOPPED HERE Genomic analysis ########################
#Plot Fisher's exact Pvalues versus MutSigCV2 output (FIGURE 1D)
...
#Used p-threshold of 0.17 in order to color FOXA1
ggplot() +
        geom_point(data = MutSig_pvals, aes(x = -log10(mutsig_p), y = -log10(fisher_p_result), size = n_patients)) +
        geom_point(data = MutSig_pvals[MutSig_pvals$fisher_p_result < 0.17, ],
                   aes(x = -log10(mutsig_p), y = -log10(fisher_p_result), color = Enriched_in, size = n_patients)) +
       # scale_color_manual(values = c(red, blue), name = "Enriched in:") +
        scale_size_continuous(name = 'Patients with alteration') +
        geom_text(data = MutSig_pvals[MutSig_pvals$fisher_p_result < 0.17, ],
